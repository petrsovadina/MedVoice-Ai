...
export const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({
  reports, onReportChange, providerConfig, entities, validationResults, onSave, isSaving
}) => {
          ...
          {activeTab === 'report' && (
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-6">
                ...
                <button onClick={onSave} disabled={isSaving} className="flex items-center gap-2 px-5 py-2.5 bg-emerald-500 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-md w-full sm:w-auto justify-center disabled:opacity-70 disabled:cursor-not-allowed">
                  {isSaving ? <Activity size={14} className="animate-spin" /> : <Save size={14} />}
                  {isSaving ? 'Ukládám...' : 'Uložit do Historie'}
                </button>
              </div>
