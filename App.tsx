...
import { AppState, AudioFile, ProcessingResult, StructuredReport, ReportType, ValidationResult, ProviderConfig, MedicalEntity, User } from './types';
...
const App: React.FC = () => {
  ...
  // Zkusíme načíst uživatele ze session storage při startu
  useEffect(() => {
    const savedUser = localStorage.getItem('medvoice_user');
    if (savedUser) {
  ...
  const handleLogin = (newUser: User) => {
    setUser(newUser);
    localStorage.setItem('medvoice_user', JSON.stringify(newUser));
    // Aktualizujeme konfiguraci jménem přihlášeného lékaře
    setProviderConfig(prev => ({ ...prev, name: newUser.name }));
  };
  ...
  return (
    <div className="min-h-screen flex flex-col font-inter bg-slate-50">
      <Header onOpenSettings={() => setIsSettingsOpen(true)} user={user} onLogout={handleLogout} />
